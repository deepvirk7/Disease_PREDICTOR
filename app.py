...
# Suppress the specific FutureWarning for downcasting
import warnings
...
disease_mapping = {'Fungal infection':0,'Allergy':1,'GERD':2,'Chronic cholestasis':3,'Drug Reaction':4,
...
df['prognosis'] = df['prognosis'].infer_objects(copy=False)  # Fix for the warning
...
tr['prognosis'] = tr['prognosis'].infer_objects(copy=False)  # Fix for the warning
...
def reset_symptoms():
    """Reset all symptom selections and clear results"""
...
w2 = Label(root, justify=LEFT, text="A Project by Deep Inder Singh Virk", fg="white", bg="blue")
...
# Reset button
reset_btn = Button(root, text="Reset", command=reset_symptoms, bg="red", fg="white")
reset_btn.grid(row=11, column=3, padx=10)
